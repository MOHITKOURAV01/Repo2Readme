...
def run(url, local, output, force, backup, create_dirs, include_patterns, exclude_patterns, max_file_size_kb, dry_run, strict, respect_gitignore, max_workers, provider, model, base_url, branch):
    ...
    if output is not None:
        try:
            ...
            # fail() escapes the message itself.
            fail(str(e), ExitCode.USAGE, printer=notify)
    ...
    with Progress(console=status_console()) as progress:
        ...
        try:
            ...
            fail(f"Failed to load repository: {e}", ExitCode.FAILURE, printer=notify)
    ...
    if dry_run:
        ...
        for doc in documents:
            ...
            rprint(f"✓ [green]{safe(rel_path)}[/green]")
        if skipped:
            ...
            for reason in reason_order:
                if reason in skip_reasons:
                    printed.add(reason)
                    rprint(f"{safe(f'{reason:30s}')}: {skip_reasons[reason]}")
            for reason in sorted(skip_reasons):
                if reason not in printed:
                    # A skip reason can be an OS error, which quotes the path.
                    rprint(f"{safe(f'{reason:30s}')}: {skip_reasons[reason]}")
    ...
    try:
        ...
        if output_target is None:
            # The README is written to stdout verbatim. It is the product of the
